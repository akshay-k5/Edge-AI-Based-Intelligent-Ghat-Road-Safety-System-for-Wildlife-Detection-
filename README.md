# Edge-AI-Based-Intelligent-Ghat-Road-Safety-System-for-Wildlife-Detection-
This project proposes an Edge AI–based Smart Ghat Road Safety System that uses IoT, computer vision, and real-time communication to identify wildlife crossings. The system automatically sends alerts to drivers and relevant authorities, helping to improve road safety, reduce wildlife collisions, and enable faster emergency response.


# 🦁 Animal Detection Edge Node with LoRa Communication

<div align="center">

![Raspberry Pi](https://img.shields.io/badge/Raspberry%20Pi-3%20Model%20B+-A22846?style=for-the-badge&logo=Raspberry%20Pi&logoColor=white)
![Python](https://img.shields.io/badge/Python-3.7+-3776AB?style=for-the-badge&logo=python&logoColor=white)
![YOLOv8](https://img.shields.io/badge/YOLO-v8-00FFFF?style=for-the-badge&logo=yolo&logoColor=black)
![LoRa](https://img.shields.io/badge/LoRa-HPD13A-00BFFF?style=for-the-badge&logo=lora&logoColor=white)
![ESP32](https://img.shields.io/badge/ESP32-Receiver-E7352C?style=for-the-badge&logo=espressif&logoColor=white)

**Real-time Wildlife Monitoring & Alert System with Long-Range Communication**

</div>



## 🔭 Overview

An intelligent edge computing system that uses **computer vision** and **AI** to detect animals in real-time, classify them as wild or domestic, and transmit alerts over **long-range LoRa communication**. Built for remote wildlife monitoring, farm security, and conservation efforts.

<div align="center">
  
